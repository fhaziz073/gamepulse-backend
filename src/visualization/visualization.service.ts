...
export class VisualizationService {
  ...
  private applyOpponentTeamFilter(
    ...
    return rows.filter(
      (row) =>
        this.getOpponentTeamIdForEntry(row, entityId, isTeam) ===
        opponentTeamId,
    );
  }
    ...
    for (const game of sorted) {
      ...
      out.push({
        ...
        player: {
          id: playerId,
          team_id: teamId,
        } as unknown as RawStatEntry['player'],
  ...
  private extractHomeVisitorIds(game: RawStatEntry['game'] | undefined): {
    ...
    const homeFromNested =
      ht &&
      typeof ht === 'object' &&
      typeof (ht as { id?: unknown }).id === 'number'
        ? (ht as { id: number }).id
        : undefined;
    const visitorFromNested =
      vt &&
      typeof vt === 'object' &&
      typeof (vt as { id?: unknown }).id === 'number'
        ? (vt as { id: number }).id
        : undefined;
    const homeId =
      homeFromNested ??
      (typeof g['home_team_id'] === 'number' ? g['home_team_id'] : undefined);
    const visitorId =
      visitorFromNested ??
      (typeof g['visitor_team_id'] === 'number'
        ? g['visitor_team_id']
        : undefined);
  ...
  private getPlayerStatTeamId(entry: RawStatEntry): number | undefined {
    ...
    if (typeof entry.stats?.team_id === 'number')
      return Number(entry.stats.team_id);
    return undefined;
  }
  ...
  private resolveOpponentFromEntry(
    ...
    if (
      ...
      entry.opponentName.trim().length > 0
    ) {
    ...
    if (isTeam) {
      if (entityId === homeId) {
        return {
          opponentName:
            typeof visitorName === 'string' ? visitorName : undefined,
  ...
  private async resolveTeamFullName(
    teamId: number,
  ...
  private teamNameForScoreLine(team?: {
    id?: number;
    abbreviation?: string;
    full_name?: string;
  }): string {
  ...
  private async preloadTeamNamesFromRawEntries(
    rows: RawStatEntry[],
  ): Promise<void> {
  ...
  private async preloadTeamNamesFromGameList(
    games: GameEntry[],
  ): Promise<void> {
    const ids = new Set<number>();
    for (const game of games) {
      if (typeof game.home_team?.id === 'number') ids.add(game.home_team.id);
      if (typeof game.visitor_team?.id === 'number')
    ...
    if (statKey === 'pts') {
      ...
      for (const game of h2h) {
        ...
        rawA.push({
          ...
          game: gPayload,
          opponentName:
            (isHomeA ? game.visitor_team : game.home_team)?.full_name ??
            undefined,
        } as RawStatEntry);
        rawB.push({
          date: this.normalizeIsoDate(game.date),
          ...
          game: gPayload,
          opponentName:
            (isHomeA ? game.home_team : game.visitor_team)?.full_name ??
            undefined,
        } as RawStatEntry);
      }
      return {
